...
- Evaluate on VQA-RAD's **official 451-example held-out test split** (not a re-split), so results are directly comparable to other published VQA-RAD baselines.
- Further divide the **official training portion (1,793 examples) 90/10** into internal train/validation sets, stratified by answer type (closed vs. open) where that label is available.
